...
drupal_add_js(drupal_get_path('theme', 'jarvis') . '/js/lib/modernizr.js');
$inline_js = 'var polyfilter_scriptpath = "'.drupal_get_path('theme', 'jarvis').'/js/lib/";';
drupal_add_js($inline_js,'inline');
drupal_add_css(drupal_get_path('theme', 'jarvis') . '/css/theme-settings.css', array('group' => CSS_THEME));
drupal_add_css(drupal_get_path('theme', 'jarvis') . '/js/lib/colorpicker/css/colorpicker.css', array('group' => CSS_THEME));
drupal_add_css(drupal_get_path('theme', 'jarvis') . '/js/lib/css/modal-default.css', array('group' => CSS_THEME));
drupal_add_css(drupal_get_path('theme', 'jarvis') . '/js/lib/css/modal-component.css', array('group' => CSS_THEME));
drupal_add_css(drupal_get_path('theme', 'jarvis') . '/js/lib/css/bootstrap-dialog.css', array('group' => CSS_THEME));
...
drupal_add_js(drupal_get_path('theme', 'jarvis') . '/js/lib/jquery.cookie.js');
...
drupal_add_js(drupal_get_path('theme', 'jarvis') . '/js/lib/colorpicker/js/colorpicker.js');
drupal_add_js(drupal_get_path('theme', 'jarvis') . '/js/lib/jquery.leanModal.min.js');
drupal_add_js(drupal_get_path('theme', 'jarvis') . '/js/lib/bootstrap-dialog.js');
drupal_add_js(drupal_get_path('theme', 'jarvis') . '/js/lib/jquery.choosefont.js');
drupal_add_js(drupal_get_path('theme', 'jarvis') . '/js/lib/classie.js');
drupal_add_js(drupal_get_path('theme', 'jarvis') . '/js/lib/modalEffects.js');
drupal_add_js(drupal_get_path('theme', 'jarvis') . '/js/lib/cssParser.js');
drupal_add_js(drupal_get_path('theme', 'jarvis') . '/js/lib/css-filters-polyfill.js');

drupal_add_js(drupal_get_path('theme', 'jarvis') . '/js/lib/popup-content.js');

drupal_add_js(drupal_get_path('theme', 'jarvis') . '/js/lib/theme-settings.js');

require_once DRUPAL_ROOT . '/' . drupal_get_path('theme', 'jarvis') . '/inc/theme-settings-general.inc';
require_once DRUPAL_ROOT . '/' . drupal_get_path('theme', 'jarvis') . '/inc/theme-settings-design.inc';
require_once DRUPAL_ROOT . '/' . drupal_get_path('theme', 'jarvis') . '/inc/theme-settings-text.inc';
require_once DRUPAL_ROOT . '/' . drupal_get_path('theme', 'jarvis') . '/inc/theme-settings-display.inc';
require_once DRUPAL_ROOT . '/' . drupal_get_path('theme', 'jarvis') . '/inc/theme-settings-pages.inc';
require_once DRUPAL_ROOT . '/' . drupal_get_path('theme', 'jarvis') . '/inc/theme-settings-code.inc';
require_once DRUPAL_ROOT . '/' . drupal_get_path('theme', 'jarvis') . '/inc/theme-settings-config.inc';
...
function jarvis_form_system_theme_settings_alter(&$form, &$form_state, $form_id = NULL, $no_js_use = FALSE) {
    if(isset($form_id)){
      return;
    }
    $form['jarvis_settings']['html_header'] = array(
        '#markup' => '
<!--<div class="md-links">
...
<div class="md-wrap">
  <div id="md-tabs">
    ...
    jarvis_theme_settings_general($form, $form_state);
    ...
    jarvis_theme_settings_design($form, $form_state);
    jarvis_theme_settings_text($form, $form_state);
    jarvis_theme_settings_display($form, $form_state);
    jarvis_theme_settings_code($form, $form_state);
    jarvis_theme_settings_config($form, $form_state);

    $form['jarvis_settings']['html_footer'] = array(
        '#markup' => '
	</div><!-- /#md-tabs -->
</div><!-- /.md-wrap -->',
        '#weight' => 99,
    ...
    $form['reset']      = array(
        ...
        '#submit'       => array('jarvis_reset_settings_submit'),
        '#weight'       => '1000',
    );

   $form['#submit'][] = 'jarvis_save_settings';
   $form['#validate'][] = 'jarvis_validate_theme_settings';
}

/**
 * Custom validation for jarvis theme setting
 */
function jarvis_validate_theme_settings($form, &$form_state) {
    // Header video link
 ...
 */
function _jarvis_process_google_web_font($fonts) {
 ...
 */
function _jarvis_process_typekit_font($typekit_id) {
...
function load_font_configure() {
    ...
    if ($google_font != '') {
        $result = _jarvis_process_google_web_font($google_font);
        add_font_style($result, $fontarray, $fontvars);
    }
    $typekit = theme_get_setting('typekit_id');;

    if ($typekit != '') {
        $result = _jarvis_process_typekit_font($typekit);
        add_font_style($result, $fontarray, $fontvars);
    }
...
function jarvis_save_settings($form, &$form_state) {
	...
	$form_state['values'] = saveImage('design_static_bg_path','design_static_bg_upload',$form_state['values']);
	$form_state['values'] = saveImage('map_marker_path','map_marker_upload',$form_state['values']);
	$form_state['values'] = saveImage('map_logo_path','map_logo_upload',$form_state['values']);
 ...
 */
function jarvis_reset_settings_submit($form, &$form_state){
    $theme = variable_get('theme_default');
    $theme_settings = variable_get('theme_'.$theme.'_settings');
    $default_settings = _jarvis_theme_default_settings($theme_settings);
 ...
 */
function jarvis_backup_theme_settings() {
    $theme = variable_get('theme_default');
    $theme_settings = variable_get('theme_'.$theme.'_settings');
    $current_time = time();
 ...
 */
function jarvis_restore_theme_settings($form, &$form_state) {
    $theme = variable_get('theme_default');
 ...
 */
function _jarvis_theme_default_settings ($theme_settings) {
    ...
    $default_settings['footer_text'] = 'jarvis™ design by
